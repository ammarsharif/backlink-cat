...
import { fetchWhyMarketers } from '@/lib/whyChooseService';
import { WhyChooseEmpty } from '@/components/sections/WhyChooseEmpty';

export async function WhyMarketers() {
  const data = await fetchWhyMarketers();

  if (!data) {
    return <WhyChooseEmpty title="Why Marketers Choose Us?" />;
  }
