    ...
    <section className="py-20 bg-white border-y border-[#F0F0F0] overflow-hidden">
      <Container>
        ...
        <div className="relative w-full overflow-hidden pause-on-hover">
          ...
          <div className="flex items-center gap-12 md:gap-20 w-max animate-marquee">
            {SCROLL_LOGOS.map((pub, idx) => (
              <div
                key={`${pub.id}-${idx}`}
                className="flex items-center gap-2.5 grayscale hover:grayscale-0 transition-all duration-300 opacity-50 hover:opacity-100 group cursor-pointer"
              >
            ...
            {/* Repeat the list for seamless loop if needed, though SCROLL_LOGOS already has 3x */}
          </div>
        </div>
      </Container>
    </section>
